...
def reduceH5file(pathToData, pathToDataWithoutCOMPASname, DCOtype, optimistic, BPSmodelName):
    ...
    filesOfInterest   = {1:'doubleCompactObjects',\
                             3:'supernovae',  5:'RLOF'} #{1:'doubleCompactObjects',2:'systems', 3:'supernovae'}
    ...
    columnsOfInterest =    {1:[  'M1', 'M1ZAMS', 'M2', 'M2ZAMS', 'Metallicity1',  \
                                ...
                                'separationDCOFormation', 'separationInitial', 'stellarType1', 'stellarType2', 'tc', 'tform', 'thetaSupernova1', 'weight', 'RLOFSecondaryAfterCEE'],\
                            ...
                            3:['MassCOCoreSN', 'MassCoreSN', 'MassStarCompanion', 'MassStarSN','randomSeed',  'separationAfter', 'separationBefore',  'systemicVelocity', 'whichStar'],\
                            # 4:['All'],\
                             5:['ID', 'eventCounter','eventCounterPrev', 'flagCEE', 'flagRLOF1', 'flagRLOF1Prev', 'flagRLOF2', 'flagRLOF2Prev', 'mass1', 'mass1Prev', 'mass2', \
                             'mass2Prev', 'radius1', 'radius1Prev', 'radius2', 'radius2Prev', 'randomSeed', 'separation', 'separationPrev', 'time', 'timePrev', \
                             'type1', 'type1Prev', 'type2', 'type2Prev']\
                             }
    ...
    pathToNewData = pathToDataWithoutCOMPASname + 'COMPASCompactOutput_redshift' + DCOtype + '_' + BPSmodelName +'.h5'
    ...
    # append SEEDS 
    SEED = fdata['doubleCompactObjects']['seed'][...].squeeze()
    RdetSEED = RdetPerRedshift.create_dataset(u"seed", data=SEED)
    RdetSEED.attrs[u"units"] = u"#"
    RdetSEED = RintPerRedshift.create_dataset(u"seed", data=SEED)
    RdetSEED.attrs[u"units"] = u"#" 
    ...
    for ind_zz in range(len(Data.PerSystemPerRedshift_ratesIntrinsic)):
            ...
            # append total observed rate (integrated over redshift horizon LIGO/VIRGO/KAGRA)
            Rdetxyz = RdetPerRedshift.create_dataset(u"R_%s_z_%s"%(mssfrName,zz), data=weightsDet_zz)
            Rdetxyz.attrs[u"units"] = u"yr^{-1}"
            # append total intrinsic rate at redshift 0 
            Rdetxyz = RintPerRedshift.create_dataset(u"R_%s_z_%s"%(mssfrName,zz), data=ratesPerSystem_zz)
            Rdetxyz.attrs[u"units"] = u"Gpc^{-3}yr^{-1}"             
...
#     pathToData = '/Volumes/Andromeda2/DATA/AllDCO_bugfix/fiducial/COMPASOutput.h5'
#     pathToDataWithoutCOMPASname = '/Volumes/Andromeda2/DATA/AllDCO_bugfix/fiducial/'
#     optimistic=int(0)
#     BPSmodelName = 'A'
#     print()
#     print('-------------------------------------------------------')
